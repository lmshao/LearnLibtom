...
int AES_CTR(const uint8_t *key, uint32_t keyLen, const uint8_t *iv, uint32_t ivLen, const uint8_t *inData, uint32_t inLen,
            uint8_t **outData, uint32_t *outLen, CryptoOperation cryptoType) {
    ...
    if (CRYPT_OK != ctr_start(idx, actualIV, actualKey, 16, 0, CTR_COUNTER_LITTLE_ENDIAN, &ctr)) {
        printf("AES_ECB ecb_start err.\n");
        return -1;
    }
    ...
    if (ENCRYPTION == cryptoType){
        ...
        if (CRYPT_OK != ctr_encrypt(inData, oData, inLen, &ctr)) {
            printf("AES_ECB_Enc ecb_encrypt err.\n");
            return -1;
        }
    }
        ...
        if (CRYPT_OK != ctr_decrypt(inData, oData, inLen, &ctr)) {
            printf("AES_ECB ecb_decrypt error.\n");
            return -1;
        }
    }

    if (CRYPT_OK != ctr_done(&ctr)) {
        printf("AES_ECB_Enc ecb_done err.\n");
        return -1;
    }
        ...
        if (padValue > 0 && padValue <= 16){
            ...
            for (int i = 0; i < padValue; ++i) {
                if (*(in+inLen-1-i) != padValue){
                    isPadding = false;
                    printf("AES_UnPadding input date is wrong");
                    break;
                }
            }
            if (isPadding)
